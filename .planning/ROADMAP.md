...
**Plans:** 16 plans in 8 waves (Waves 0–4 shipped initial implementation; Waves 5–8 close 9 BLOCKERs + 5 WARNINGs from 04-VERIFICATION.md / 04-REVIEW.md — concerns A–G grouped per Gaps Summary; gap-closure waves sequenced to avoid messages/*.json merge conflicts per plan-checker revision).
...
**Wave 6 — Gap closure (depends on Wave 5 file ownership)** *(closing CR-03 + CR-04)*
- [ ] 04-12-PLAN.md — Concern C (tournament result-route + RBAC — CR-03 + CR-04, depends on 04-10): tournaments/[eventId]/result/page.tsx accepts ?playerId for non-player callers (Pick-Player selector backed by extended tournament.get.participants — per plan-checker WARNING-3 Path A: +5-LOC SELECT joining calendar_event_participants + users); leaderboard + participants panel emit ?playerId-bearing navigation links; listPendingForPlayer enforces explicit role allowlist (player/trainer/TD/parent) + parent_child_links active-link probe; integration RBAC matrix 9 cells + Playwright e2e for TD-without-playerId picker fallback (4 scenarios including first-result-of-tournament regression)

**Wave 7 — Gap closure (depends on Wave 6 — messages/*.json merge after 04-12)** *(closing CR-05 + WR-09)*
- [ ] 04-13-PLAN.md — Concern D (recurring edit past-immutability + needsScoring per-occurrence — CR-05 + WR-09, depends on 04-10 + 04-12): all_in_series branch adds Brussels-anchored past-startsAt guard with writeAuditOutsideTx denial; new errors.calendar.cannotMoveSeriesToPast i18n key in nl/en/fr (catalog merge after 04-12's tournament.* additions — disjoint paths); calendar.list needsScoring SQL refactored from per-event aggregate to per (event_id, occurrence_date) aggregate; 2 integration tests with own-fixture-plant pattern (recurringTrainingEventId planted in beforeAll with deterministic rrule + dtstart — phase4-seed does NOT expose this fixture)

**Wave 8 — Gap closure (depends on Wave 7 — messages/*.json merge after 04-13)** *(closing CR-08 + WR-05/06/07/10)*
