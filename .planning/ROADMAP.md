...
**Plans:** 16 plans in 9 waves (Waves 0–4 shipped initial implementation; Waves 5–9 close 9 BLOCKERs + 5 WARNINGs from 04-VERIFICATION.md / 04-REVIEW.md — concerns A–G grouped per Gaps Summary; gap-closure waves sequenced to avoid messages/*.json merge conflicts AND parallel-worktree writes to training.ts/tournament.ts per plan-checker iteration-2 WARNING-1 resolution).
...
**Wave 5 — Gap closure (parallel; independent file sets)** *(closing CR-01/02/06/07)*
- [x] 04-10-PLAN.md — Concern A (audit forensics — CR-01): writeAuditOutsideTx helper in audit.ts + apply to 4 denied-audit call sites (training:170 / tournament:602 / calendar:1738 + 1822) + integration probe verifying denied audit rows survive rollback (T-04-19 / T-04-23 forensic visibility property delivered)
- [x] 04-11-PLAN.md — Concern B (idempotency input binding — CR-02): drizzle/0021 additive ALTER adding request_hash column; idempotencyMiddleware computes sha256(canonicalised JSON input) on every call, persists on cache MISS, compares on cache HIT, raises CONFLICT errors.idempotency.inputMismatch on mismatch; integration test covers replay / CONFLICT / MISS / key-order canonicalisation
- [x] 04-14-PLAN.md — Concern E (inbox INSERT policy + dedup — CR-06 + CR-07): drizzle/0022 INSERT policy system_inbox_insert_security_definer + REVOKE INSERT,DELETE FROM app_user + partial UNIQUE INDEX uq_system_inbox_daily on (user_id, kind, Brussels-day) + drizzle/0023 CREATE OR REPLACE cron functions with ON CONFLICT DO NOTHING; `pnpm db:push --force` (non-interactive — per plan-checker WARNING-4 resolution; drizzle-kit 0.31 supports --force, plan stays autonomous: true) + 2 integration tests (policy enforcement + daily dedup)
**Wave 6 — Gap closure (depends on Wave 5 — serialise training.ts/tournament.ts writes)** *(closing CR-09 + WR-02)*
- [x] 04-16-PLAN.md — Concern G (Brussels-anchored dates — CR-09 + WR-02, depends on 04-10): replace 6 .toISOString().slice(0,10) call sites (players.ts:102 / event-detail-sheet:202 / te-scoren-overview:43 / score/page:32 / training.ts:80 / tournament.ts:126) with formatOccurrenceDate; structural unit test forbidding future regressions on Phase 4 surfaces; integration test verifying DOM-CAT-02 Brussels-anchored snapshot for evening tournaments. Moved from Wave 5 → Wave 6 per plan-checker iteration-2 WARNING-1 — Plan 04-10 also writes to training.ts (line 170) + tournament.ts (line 602); serial waves avoid parallel-worktree writes against the same files.

**Wave 7 — Gap closure (depends on Wave 6 — serialise tournament.ts writes)** *(closing CR-03 + CR-04)*
- [x] 04-12-PLAN.md — Concern C (tournament result-route + RBAC — CR-03 + CR-04, depends on 04-10 + 04-16): tournaments/[eventId]/result/page.tsx accepts ?playerId for non-player callers (Pick-Player selector backed by extended tournament.get.participants — per plan-checker WARNING-3 Path A: +5-LOC SELECT joining calendar_event_participants + users); leaderboard + participants panel emit ?playerId-bearing navigation links; listPendingForPlayer enforces explicit role allowlist (player/trainer/TD/parent) + parent_child_links probe; integration RBAC matrix 9 cells + Playwright e2e for TD-without-playerId picker fallback (4 scenarios including first-result-of-tournament regression). Wave bump from 6 → 7 + depends_on extended to [10, 16] per plan-checker iteration-2 WARNING-1.

**Wave 8 — Gap closure (depends on Wave 7 — messages/*.json merge after 04-12)** *(closing CR-05 + WR-09)*
- [x] 04-13-PLAN.md — Concern D (recurring edit past-immutability + needsScoring per-occurrence — CR-05 + WR-09, depends on 04-10 + 04-12): all_in_series branch adds Brussels-anchored past-startsAt guard with writeAuditOutsideTx denial; new errors.calendar.cannotMoveSeriesToPast i18n key in nl/en/fr (catalog merge after 04-12's tournament.* additions — disjoint paths); calendar.list needsScoring SQL refactored from per-event aggregate to per (event_id, occurrence_date) aggregate; 2 integration tests with own-fixture-plant pattern (recurringTrainingEventId planted in beforeAll with deterministic rrule + dtstart — phase4-seed does NOT expose this fixture). Wave bump from 7 → 8 per plan-checker iteration-2 WARNING-1 cascade.

**Wave 9 — Gap closure (depends on Wave 8 — messages/*.json merge after 04-13)** *(closing CR-08 + WR-05/06/07/10)*
- [x] 04-15-PLAN.md — Concern F (i18n XSS sinks + hardcoded labels + chart locale — CR-08 + WR-05/06/07/10, depends on 04-12 + 04-13): replace dangerouslySetInnerHTML with t.rich + `<b>` chunks in nudge-banner.tsx + rrule-scope-picker-dialog.tsx (catalogs rewritten from `**bold**` to `<b>bold</b>`); fix WR-10 daysLeft off-by-one via Math.ceil(msLeft/86400000); add 5 i18n labels for tournament-detail page (Startdatum / Einddatum / Type / Leeftijdscategorie / Deelnemers) + translate lookup codes via existing tLookupType/tLookupAge namespaces (per plan-checker WARNING-1: namespaces already present in catalogs — no namespace seeding); ranking-line-chart uses useLocale for date formatting; structural unit test forbidding dangerouslySetInnerHTML in src/components/. Wave bump from 8 → 9 per plan-checker iteration-2 WARNING-1 cascade.
