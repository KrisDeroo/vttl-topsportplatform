...
- **D-04:** Consent-teksten in alle drie talen (operationele data, medische verwerking, foto/video-gebruik × nl/en/fr = 9 documenten) worden **team-drafted** in Fase 1 met versie `1.0.0-draft` en samen met de plans gecommit. **Geen hard gate op juridische ondertekening vóór migratie 001** — Fase 1 mag op staging draaien met draft-tekst. **Juridische review verplaatst naar Fase 8 release-gate** (samen met DPIA): vóór productie-livegang moet minstens NL juridisch ondertekend zijn (`policy_version` bumped naar `1.0.0`); EN/FR ondertekening per locale vóór die locale productie-live gaat (RISK-I18N-LEGAL gedocumenteerd, niet blokkerend voor Fase 1).
- **D-05:** EN- en FR-vertalingen worden parallel met Fase 1-implementatie aangemaakt op basis van de NL-bronteksten; ook draft-status (`1.0.0-draft`) tot Fase 8. Bij text-correcties tijdens Fase 1-ontwikkeling: bump `policy_version` (bv. `1.0.1-draft`); oude draft-consents in dev/staging blijven via snapshot zichtbaar maar dev-DB wordt periodiek geleegd dus geen migratie-impact.
